...
<a href=""><img class="ico_follow-er" src="{{ asset('/img/ico_follow.png') }}"></a>

<a href=""><img class="ico_follow-er" src="{{ asset('/img/ico_follower.png') }}"></a>
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-black-500 bg-white hover:text-gray-700 focus:outline-none transition ease-in-out duration-150 ">
                            <div class="text-right" style="width: 100px;">@if(Auth::check()){{ Auth::user()->name }}@endif</div>
...
<div id="description">SpoNS（スポンズ）で、スポーツ仲間を見つけよう！</div>
<div class="categories">
    <ul class="flex">
        @foreach($categories as $category)
        <li><a href=""><img src="{{ asset("/img/{$category->image}") }}" class="pictogram">{{ $category->name }}</a></li>
        @endforeach
